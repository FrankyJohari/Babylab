...
$id = $noid['id_penerimaan']+1;
$finalid = "IN/".date('y')."/".strtoupper(date('M'))."/".$id;
?>

<div class="card">
  <h5 class="card-header">Penerimaan Kas</h5>
  <div class="card-body">
   	<form method="POST">
	...
	<br>
  	...
  	<div class="form-row">
		...
		<?php
		$data = mysqli_query($connect,"select * from account where main_acc='200' OR main_acc='300' OR main_acc='400' ");
	...
	<div class="form-row">
      ...
      <div class="form-row">
  		<div class="col">
		<label>Total</label>
		<input type="text" name="total" placeholder="Total" id="harga" class="form-control" autocomplete="off" required>
		<br>
  		</div>
  	</div>
	<button type="submit" name="simpan_penerimaan" class="btn btn-warning btn-lg btn-block">Save</button>
   	</form>
  </div>
</div>
